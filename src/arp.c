...
static int update_arp_tranlsation_table(struct arp_hdr *hdr, struct arp_ipv4 *data)
{
...
static int insert_arp_tranlsation_table(struct arp_hdr *hdr, struct arp_ipv4 *data) 
{
    struct arp_cache_entry *entry;
    for (int i = 0; i < ARP_CACHE_LEN;i++)
    {
...
void arp_incoming(struct netdev *netdev, struct eth_hdr *hdr)
{
    ...
    {
        printf("Unsopported HW type\n");
        return;
    }

    if (arphdr->protype != ARP_IPV4)
    {
        printf("Unsopported protocol \n");
        return;
    }

    arpdata = (struct arp_ipv4 *) arphdr->data;
    
    merge = update_arp_tranlsation_table(arphdr,arpdata);

    if (netdev->addr != arpdata->dip)
    {
        printf("ARP was not for us \"how\" \n");
        return;
    }
    ...
    if (!merge && insert_arp_tranlsation_table(arphdr,arpdata) != 0)
